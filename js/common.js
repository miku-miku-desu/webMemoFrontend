...
/****************************************************/
function onload() {
	if (!localStorage.getItem("update_time")) {
		localStorage.setItem("update_time", "0");
	}
	if (!localStorage.getItem("memoireData")) {
		localStorage.setItem("memoireData", "[]");
	}
	let username = localStorage.getItem("username");
	if (username == null) {
		$("#username").text("点击登录").click(() => {
			location.href = "user/";
		});
		$("#autoSync").click(() => {
			location.href = "user/";
		...
		$("#logout").css("display", "block").click(() => {
...
let autoSync = () => {
	let memo = localStorage.getItem("memoireData");
	let time = localStorage.getItem("update_time");
	let host = localStorage.getItem("host");
	let token = localStorage.getItem("token");
	let username = localStorage.getItem("username");
	if (memo == null || time == null || host == null || token == null || username == null) {
		...
		success: (res) => {
			if (res.status === true) {
				if (res.body.result) {
					...
					localStorage.setItem("memoireData", res.body.memo_content);
					localStorage.setItem("update_time", res.body.time.toString());
					alert("本地同步成功");
					location.reload();
				}
			} else {
...
function markListItem() {
	$("#memoireList").on("click", "li > div", function (e) {
		...
		localStorage.setItem("update_time", Date.now().toString());
